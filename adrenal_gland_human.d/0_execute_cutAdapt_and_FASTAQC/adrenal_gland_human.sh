...
#Execute cutadapt and fastqc
for i in "${array[@]}"
do
        cd qcReads.d/$smpl/rawdata/$i
        fastqIn="${i}_R1.fastq.gz"
        fastqTrmmd="${i}_R1_trimmed.fq.gz"
        rprtFl="${i}_R1.fastq.gz_trimming_report.txt"
        cutadapt -f fastq -e 0.1 --quality-base=64 -O 1 -a CTGTCTCTTATA $fastqIn -m 20 -o $fastqTrmmd > $rprtFl
        fastqc $fastqTrmmd >> $rprtFl
done
